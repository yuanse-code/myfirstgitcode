...
//向前声明
static void FindHeadRoot(BTNode*** pproot, BTNode* root);


static size_t printNum = 0;
static void ShowByPeoInfo(BTDataType x)
{
	FILE* bufferPf = fopen("缓冲.txt", "a");

	int year1; int month1; int day1;
	int year2; int month2; int day2;
	day1 = x->makeDay % 100; month1 = (x->makeDay % 10000) / 100;
	year1 = x->makeDay / 10000;
	day2 = x->lastDay % 100; month2 = (x->lastDay % 10000) / 100;
	year2 = x->lastDay / 10000;
	fprintf(bufferPf, "%2d\t%-12d\t%-10s\t%-4d\t%-5s\t%-20s\t%-20s\t%d年%d月%d日\t%d年%d月%d日\n",
		printNum + 1, x->vipNum, x->name, x->age,
		x->sex, x->tale, x->addr,
...
void Insert(BTNode** proot, BTDataType x)
{
	...
	else if ((*proot)->_data->vipNum >= x->vipNum)//比父节点小，插入左边
	{
		if ((*proot)->pLeft)
...
static BTNode* FindEorrorNode(BTNode* root)
{
...
BTNode* SearchNode(BTNode* root, BTDataType x)
{
	...
	if (x == root->_data)
		return root;
	else if (x->vipNum > root->_data->vipNum)//比root大
	{
